...
> **实现说明**：本节最早的设计构想是 SQLite + FTS5，但正式实现**不使用任何数据库**。
> 实际存储为纯 JSON 文件（v2 布局）：`index.json`（项目元信息 + 策略 + 做梦记录）+ `sessions/*.json`
> （按会话拆分记忆节点），检索在内存中完成（BM25 / 余弦 / 文件重合，见 `src/lib/scoring.ts`），
> 旧版单文件 `memory.json` 启动时自动迁移。以下 SQL 仅为早期方案的记录，不构成当前实现。
