...
test("dream_memory_search defaults limit to policy maxRecall, not 1", async () => {
    const store = await loadStore()
    commitN(store, 7, { summary: "fix the billing amount overflow", files: ["src/billing.ts"] })
    const text = await tools(store).dream_memory_search.execute({ query: "billing" }, ctx())
...
test("dream_memory_search clamps explicit limits to 1..10", async () => {
    const store = await loadStore()
    commitN(store, 12, { summary: "schema migration", files: ["src/db.ts"] })
    const text = await tools(store).dream_memory_search.execute({ query: "migration", limit: 0 }, ctx())
